...
export function OnrampCheckoutCard({ card, onPaid }: OnrampCheckoutCardProps) {
  ...
  return (
    <ChatCard className="border border-jumpa-neutral-100 bg-jumpa-white">
      ...
      {card.reference && (
        <p className="text-center text-xs text-jumpa-black/50">
          Payment Reference:{" "}
      ...
      {!isDone && (
        <button
          ...
          disabled={verifying}
          className="mt-1 flex h-9.5 w-full items-center justify-center rounded-pill bg-jumpa-primary-600 text-[13px] font-semibold text-jumpa-white transition-opacity hover:bg-jumpa-primary-700 active:scale-98 disabled:opacity-50 cursor-pointer shadow-xs"
        >
          {verifying ? (
