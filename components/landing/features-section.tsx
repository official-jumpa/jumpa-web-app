 ...
 *
 * The cards scale up rather than sliding: the row is an `overflow-x-auto`
 * scroller, and a card that starts translated would widen its own scrollable
 * area on the way in. A card that starts smaller can never add overflow.
 */
  ...
  return (
    <section id="features" className="pt-34 md:pt-128">
      <div className="mx-auto flex w-350 flex-col gap-10 md:w-1300 md:flex-row">
        <div className="reveal-left flex h-267.5 shrink-0 flex-col justify-center gap-15 rounded-u-30 bg-jumpa-primary-25 p-20 md:h-615 md:w-548 md:gap-50 md:px-50">
        ...
        <div className="flex gap-17.5 overflow-x-auto rounded-u-30 bg-jumpa-primary-25 p-20 md:gap-40 md:px-50 md:py-54 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <SendCard className="reveal-zoom" style={revealStep(0)} />
          <ReceiveCard className="reveal-zoom" style={revealStep(1)} />
          <SwapCard className="reveal-zoom" style={revealStep(2)} />
          <SaveCard className="reveal-zoom" style={revealStep(3)} />
        </div>
      </div>
    </section>
  );
